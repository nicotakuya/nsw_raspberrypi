...
def selectcard(cursor):
	...
	screen.blit(textimage, (cardx(5)-14, cardy(plmy)-10))
#カードを捨てる
def killcard(cursor):
	n = crd[plmy][cursor]
	if n==-1:return	
...
def getlevel(pl):
	...
	num = crd[pl][0] % 13
	for i in range(1,5):   #連続しているか
		if (num+i)!=(crd[pl][i] % 13) : strgt=0
		if mark != int(crd[pl][i]/13)   : flash=0

	if strgt!=0 and flash!=0 :
		if num==8 : return 9   #ロイヤルストレートフラッシュ
		return 8+(num/100)     #ストレートフラッシュ
	...
	for i in range(4):
		...
		if (n % 13)==(n2 % 13) : #ペア発見
			...
			s=(n % 13)/100
			cnt += 1
			if card<cnt :card=cnt  #カード枚数更新
		else:
			cnt=1  #ペアじゃない

	if card==4 :return 7+s               #4カード
	if card==3 and pair==2 :return 6+s   #フルハウス
	if flash!=0 :return 5+(num/100)      #フラッシュ
	if strgt!=0 :return 4+(num/100)      #ストレート
	if card==3 and pair==1 : return 3+s  #3カード
	if card==2 and pair==2 : return 2+s  #2ペア
	if card==2 and pair==1 : return 1+s  #1ペア
	return 0  #ブタ
#結果表示
def result():
	for pl in range( plmax):
		...
		textimage = jfont.render(handname[hand[pl]], True, color)
		screen.blit(textimage, (cardx(0), cardy(pl)+30))
		textimage = efont.render(str(prize[pl]), True, color)
...
def setcard( pl,i,n,opened):
	...
	u = (temp % 13)*CARDW
	v = int(temp/13)*CARDH
	sp[n].image = pygame.Surface.subsurface(cardimage,(u,v,CARDW,CARDH))
#カードをランダムで引く
def rndcard():
	sp = allgroup.sprites()
	while 1:
...
jfont = pygame.font.Font("C:\Windows\Fonts\MSGOTHIC.TTC", 24)
...
handname=[
	...
	"ロイヤルストレートフラッシュ"]
crd = [[0 for i in range(5)] for j in range(plmax)]
...
while endflag==0:
	...
	for pl in range( plmax):
		textimage = efont.render(username[pl], True, WHITE)
		screen.blit(textimage, (80, cardy(pl)-30))
		textimage = efont.render(str(score[pl]), True, YELLOW)
