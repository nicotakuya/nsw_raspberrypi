...
class Player(Spclass):
	def update(self):
		...
		if(press[pygame.K_DOWN]) :self.rect.centery += 4
		if(press[pygame.K_LEFT]) :self.rect.centerx -= 4
		if(press[pygame.K_RIGHT]):self.rect.centerx += 4
		if(self.rect.centerx<16):self.rect.centerx = 16
		if(self.rect.centery<16):self.rect.centery = 16
		if(self.rect.centerx>WIDTH-16 ):self.rect.centerx = WIDTH-16
		if(self.rect.centery>HEIGHT-16):self.rect.centery = HEIGHT-16
...
class Boss(Spclass):
	def update(self):
		...
		self.rect.centerx = (WIDTH/2)+(math.sin(rad)*200)
		if(self.time % 3==0):
...
charas = []
charas.append(Characlass("chr0.png",1 ,False))   #0:explosion
charas.append(Characlass("chr1.png",1 ,False))   #1:shot
charas.append(Characlass("chr2.png",99,True))    #2:fireball
charas.append(Characlass("chr3.png",1 ,False))   #3:player
charas.append(Characlass("chr4.png",1 ,True))    #4:fighter
charas.append(Characlass("chr5.png",10,True))    #5:boss
stars = []
for i in range(10):
